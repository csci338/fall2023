...
>
> 1. [Modifying your Docker compose file to install PostgreSQL](#installation)
> 1. [Learning some basic SQL Rules](#sql) to create, read, update, and delete (CRUD) data from your database.
> 1. [Completing the SQL exercises](#sql-practice)
> 1. [Learning about Object-Relational Mapping (ORMs)](#orms)
> 1. [Connecting your tasks backend to a database](#your task)
...
### 1.1. Create a Database script
At the root of your copy of the `lab05` directory, create a new folder called `db`. Then, within the `db` folder create a new file called `setup.sql` with the following information:
...
-- insert some rows: 
INSERT INTO public.todos(description, done) VALUES ('go to the store', false);
INSERT INTO public.todos(description, done) VALUES ('mow the lawn', false);
INSERT INTO public.todos(description, done) VALUES ('feed the dog', false);
```

This script will create a table called `todos` with three columns: `id`, `description`, and `done`, and then insert three dummy records into the table.

### 1.2. Update your python dependencies in your pyproject.toml file
Within your `pyproject.toml` file (inside the `src` directory), please add the following dependency:

```
asyncpg = "^0.28.0"
```
This new module will allow Python's `asyncio` functions to interact with PostgreSQL asynchronously.

### 1.3. Update your server.py file so that it includes database connection logic
...
Next, below your `taskdb` array and above your routes, add the following:

```py
# lazily connect to the database when needed, but don't recreate the
# connection
class Database:
    _conn = None

    @classmethod
    async def getConnection(cls):
        if Database._conn:
            return Database._conn
        else:
            Database._conn = await asyncpg.connect(
                user="postgres", password="postgres", database="postgres", host="db"
            )
            return Database._conn
...
async def lifespan(app: FastAPI):
    # nothing on startup
    yield
    # this will shut down the database connection if it has been
    # created
...
Finally, right before this line...

```py
app.mount("/", StaticFiles(directory="ui/dist", html=True), name="ui")
```

... add the following new route to your api...

```py
@app.get("/api/v1/db")
async def db_test():
    conn = await Database.getConnection()
    values = await conn.fetch("SELECT * FROM todos")
    return values
```

### 1.4. Modify Docker's compose.yaml file
Please modify your `compose.yaml` file so that it looks like the one below:
...
Some helpful Docker commands for interacting with the database are listed below:

#### Run postgres docker container locally:

```
$ docker run -p 5432:5432 -e POSTGRES_PASSWORD=postgres postgres:15
```

#### Connect to postgres running on docker locally (after installing psql)
